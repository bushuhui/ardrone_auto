...
static void send_command(int nab_sequence)
{
    ...
    snprintf(str, AT_BUFFER_SIZE, "AT*PCMD=%d,%d,%d,%d,%d,%d\r",
             nb_sequence++,
             flag,
             *(int*)&radiogp_cmd.pitch,
             *(int*)&radiogp_cmd.roll,
             *(int*)&radiogp_cmd.gaz,
             *(int*)&radiogp_cmd.yaw);
...
void at_write (int8_t *buffer, int32_t len)
{
    ...
    if( at_udp_socket < 0 ) {
        ...
        at_udp_addr.sin_addr.s_addr = inet_addr("192.168.1.2");
