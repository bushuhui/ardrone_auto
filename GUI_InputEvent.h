...
#define OSA_VK_KEYTAB_LEN   		256

/* Qt input event struct */
struct InputEvent
{
    u_int32_t		type;			/* event type */
    u_int32_t		source;			/* event source */

    u_int64_t		timestamp;		/* timestamp of event */
    u_int64_t		dt;             /* time elapsed after occured */

    u_int32_t 	    code;			/* code */
    u_int32_t       code_raw;       /* raw code */

    int32_t         x_pos;			/* mouse x corridate */
    int32_t         y_pos;			/* mouse y corridate */

    size_t          data;			/* private data */
};

// translate Qt key code to universial key code
